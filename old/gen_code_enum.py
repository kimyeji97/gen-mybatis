...
where_sql = "where cd_pid is null order by cd_id"
...
def get_code_groups(connection_opts, table_name):
    ...
    cursor.execute('SELECT * FROM {} {}'.format(table_name,where_sql),False)
    def to_lower(s):
        return s.lower()
...
def get_codes(code_group , connection_opts,table_name):
    cnx = psycopg2.connect(**connection_opts)
    cursor = cnx.cursor()
    sql = "SELECT * FROM "+table_name+" WHERE cd_pid = " + code_group.gcode + " order by cd_order::integer" #+ " and cd_pid < 9000"
    #sql = "SELECT * FROM tb_common_cd_payment"
    # print (sql)
...
def create_src_string():
    if use_surfinn is True:
        src_import = "import com.innerwave.core.types.CommonCode;"
        code_interface = ""
    else:
        src_import = """
...
"""
    ...
    import_prefix = """package com.innerwave;
...
{src_import}
import com.innerwave.core.jackson.th.*;
{add_import}
...
{{
    {code_interface}
    public static class PlatformCode implements CommonCode
    {{
        private Long pcode;
        private Long code;
...
for cg in code_groups:
    get_codes(cg , con_opts,'tb_common_cd')
...
target_folder = 'com.innerwave'
to_path = os.path.realpath(os.path.join(dir_path, '../../../innerwave-core/src/main','java',target_folder.replace('.','/')))
file_nm = 'PlatformCodes.java'
